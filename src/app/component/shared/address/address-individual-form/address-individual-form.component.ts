...
export class AddressIndividualFormComponent implements OnInit, AfterViewInit, OnChanges{
   // Contact form. 
   ...
   isAddressFormOpen = new BehaviorSubject<boolean>(false);
    ...
    deleteAddress(){
      if(this.addressSelected !== null){
        ...
        this.addresses = [...this.addresses.slice(0, index), ...this.addresses.slice(index+1, this.addresses.length)];
        this.sendContactsToParentComponent();
        this.addressSelected = this.resetAddressSelected();
        this.loadDataSource()
      }
