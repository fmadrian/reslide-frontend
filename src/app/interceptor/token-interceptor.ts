...
export class TokenInterceptor implements HttpInterceptor{
    ...
    intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        if (req.url.indexOf(ApiRoutes.auth.refreshToken) !== -1 || req.url.indexOf(ApiRoutes.auth.login) !== -1) {
        ...
        if(!this.isTokenRefreshing){
            ...
            return this.authService.refreshToken().pipe(
                switchMap((refreshTokenResponse : LoginResponse) =>{
                    ...
                    return next.handle(this.addToken(req, refreshTokenResponse.authenticationToken)); // Returns the intercepted request with the new authorization token included.
                })
            );
        }else{
